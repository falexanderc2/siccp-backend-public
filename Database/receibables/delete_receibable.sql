DROP FUNCTION IF EXISTS receibables.delete;

CREATE FUNCTION receibables.delete(IN p_idReceibable BIGINT) RETURNS JSON
AS
$$
DECLARE	
...
BEGIN
DELETE FROM receibables.receibables WHERE "idReceibable" = p_idReceibable;
GET DIAGNOSTICS rowAffect = ROW_COUNT;

  IF rowAffect>0 THEN
    success := true;
    messageInfo := 'LOS DATOS FUERON ELIMINADOS CON EXITO!.';
    return (select json_build_object('errorFound',errorFound, 'success',success, 'messageInfo',messageInfo, 'rowAffect',rowAffect, 'id',p_idReceibable));
  ELSE
    errorFound:=true;
