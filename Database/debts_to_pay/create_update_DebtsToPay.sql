DROP FUNCTION IF EXISTS "debitToPay".create_update;

CREATE OR REPLACE FUNCTION "debitToPay".create_update (
    IN p_action int,
...
BEGIN  
  ...
  IF p_action = 1 THEN
      INSERT INTO "debitToPay".debts_to_pay (
            "idEnterprise", "idCreditor",  "debtDescription",  "dateDebts", "amountDebt" ,
            "numberOfPayments", "amountToBePaidIninstallments" ,--"numberOfDaysForInstallments" ,
            "paymentStartDate",observations ,"remainingDebt"
      )
      VALUES (
          p_idEnterprise ,p_idCreditor , p_debtDescription, p_dateDebts, p_amountDebt ,
          p_numberOfPayments, p_amountToBePaidIninstallments , --p_numberOfDaysForInstallments ,
          p_paymentStartDate, p_observations ,p_amountDebt
       )RETURNING "idDebtToPay" into aux_id;
 
  ELSE
      UPDATE "debitToPay".debts_to_pay  SET 
            "idEnterprise" = p_idEnterprise, 
            "idCreditor" = p_idCreditor,  
